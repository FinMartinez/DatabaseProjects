...
from sqlalchemy.orm import relationship, sessionmaker, declarative_base, Bundle
...
if engine: 
    ...
    prog_query = session.query(exists().where(Program.name == name)).scalar()
    if prog_query == True:
        query = select(Program.name, Program.descr, Camper.name).\
                            join(Participate, Program.name == Participate.program).\
                            join(Camper, Participate.camper == Camper.id).where(Program.name == name)
        results = session.execute(query)
        for column in results:
            print(column[0] + ": " + column[1] + os.linesep + column[2])
            for names in results:
               print(names[2])
    elif prog_query == False:
