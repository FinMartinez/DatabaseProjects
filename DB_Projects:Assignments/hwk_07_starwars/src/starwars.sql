...
SELECT B.title AS title, AVG(A.rating) AS rating FROM FilmRatings A
INNER JOIN Films B
ON A.film = B.id
GROUP BY B.title 
...
SELECT COUNT(A.rating) AS number_of_ratings, ROUND(AVG(A.rating),2) AS average_rating, B.name As character FROM CharacterRatings A
INNER JOIN Characters B
ON A.character = B.id
WHERE B.name LIKE 'Princess Leia%'
GROUP BY B.name;

--Mota's answer:
SELECT COUNT(*) AS ratings, ROUND(AVG(A.rating), 2) AS average_rating
...
SELECT B.title AS title, ROUND(AVG(A.rating),2) AS rating FROM FilmRatings A
INNER JOIN Films B
ON A.film = B.id
WHERE B.title LIKE 'Star Wars: Episode V The Empire Strikes Back%'
GROUP BY B.title 
ORDER BY rating DESC
LIMIT 1;

--Mota's answer(incomplete):
...
SELECT COUNT(A.rating) AS number_of_ratings, B.name As character FROM CharacterRatings A
INNER JOIN Characters B
ON A.character = B.id
GROUP BY B.name
ORDER BY 1
...
SELECT ROUND(AVG(A.rating),2) AS rating, B.name As character FROM CharacterRatings A
INNER JOIN Characters B
ON A.character = B.id
INNER JOIN Fans C
ON A.fan = C.id
...
SELECT COUNT(A.income) AS count, B.description AS IncomeLevels FROM Fans A
INNER JOIN IncomeLevels B
ON A.income = B.seq
GROUP BY B.description;
--It doesn't appear that any one age group has 100 fans
